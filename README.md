...
> **状态边界：** 映射架构、双控制模式和执行器已经实现；小米 2 Pro 固件 2671 的方向四键、确认、返回、`TV` 和电源键均已按新格式人工确认。四个方向已通过 `No Event` 隔离、直接光标定位和真实坐标监测；点击、模式切换和电源动作仍需逐项验收，因此实体按键动作目前仍是 implementation preview。
...
该脚本只为这款遥控器把方向键、确认、返回、TV 和电源临时映射为 `No Event`，阻止 macOS 前台 App 同时执行原生键盘行为；米遥仍读取原始 Usage。看到“桥接已就绪”后按住语音键说话，说完松手。终端需保持运行；按 `Control + C` 停止并自动恢复原映射。
