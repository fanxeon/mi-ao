...
- Physical-button mode synthesizes mouse events, arrows, Return, and Escape. The eight confirmed keys first receive an exact-device HID `No Event` mapping so native keys do not reach the frontmost app; event correlation remains a secondary defense. Click, Power/Consumer Control, multi-display, and simultaneous-keyboard acceptance is not complete. Use `--no-buttons` during sensitive editing.
