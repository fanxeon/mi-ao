...
- 菜单栏加入基于 Lucide Sun 的 17 pt 专用单色模板资产；重连倒计或智能休眠时提供立即唤醒操作。
- “使用偏好”新增可立即生效的语音连接模式：默认“随时就绪”与可选“智能休眠”；Preferences schema 升级为 v3。

### Changed

- 菜单栏常态固定使用米遥品牌图标，交由 macOS Template Image 自动适配深浅菜单栏；真实指令仍短暂切换对应系统图标，但不再强制颜色或绘制圆角底色。
- 语音连接与实体按键反馈解耦：搜索、断连、重连或智能休眠时，已执行的实体按键动作仍可显示指令图标。
- “随时就绪”重连节奏改为 `1 → 2 → 4 → 8 → 16 → 32 → 60` 秒，之后保持 60 秒低频恢复；真实按键活动可打断倒计并立即重连。

### Fixed

- “智能休眠”在两次快速自动恢复后停止后台握手，由遥控器按键活动、蓝牙重新开启或菜单栏手动操作唤醒。

## [0.2.1] - 2026-07-17

### Fixed

- 设置页检查卡的“必须 / 当前功能必需 / 可选”去掉胶囊底色和固定尺寸，改为与标题同基线的纯有色文字。
- 启动门禁通过后先显示“已在菜单栏运行”的成功确认，用户点击完成后再结束向导，避免正常进程交接被误认为闪退。
- 加固菜单栏状态项生命周期：运行态先完成 `NSApplication` 启动再创建状态项，并由进程级强引用持有控制器。
- 补充刘海屏菜单栏可见区域边界：右侧常驻项过多时，米遥可能已正常运行但状态项被刘海或空间不足遮挡，不应误判为闪退。
