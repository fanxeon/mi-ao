...
米遥目前是 **source-first alpha**。安装后在 Finder 中双击 `~/Applications/米遥.app`。

双击会打开设置向导，而不是直接接管按键。向导复查六项环境后，“连接遥控器并开始”才调用 App 内置的真实启动门禁；成功后向导退出，米遥继续在菜单栏运行。日常启动不再依赖源码目录位置，当前仍没有开机自启。
...
双击 `~/Applications/米遥.app`，确认全部检查为绿色，点击“连接遥控器并开始”。普通用户无需再进入项目目录；开发者命令行备用入口仍是 `./scripts/start.sh`。
...
- 已支持首次设置向导、App 内置日常运行组件、菜单栏 GUI、安全后台启停和重复实例门禁；开机自启尚未实现；
