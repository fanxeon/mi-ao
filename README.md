...
> **状态边界：** 映射架构、双控制模式和执行器已经实现；小米 2 Pro 固件 2671 的方向四键、确认、返回、HOME、TV、电源和语音键均已按新格式人工确认。米遥接管这十键并拦截原生副作用，菜单和音量加减保留 macOS 原生行为。点击、模式切换和电源动作仍需逐项验收。
...
该脚本只为这款遥控器把方向键、确认、返回、HOME、TV、电源和语音键临时映射为 `No Event`，阻止 macOS 前台 App 同时执行原生键盘行为；菜单和音量加减保持原生，米遥仍读取被接管键的原始 Usage。按 `Control + C` 停止并自动恢复原映射。
