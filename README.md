...
在设置向导的「按键配置」页可以从默认配置新建或复制方案，逐键选择内置动作或录制标准键盘快捷键，再保存并选择为下一次启动使用的配置。默认 `pointer` 保持只读；语音键始终按住说话，菜单键始终保留 macOS 原生右键。自定义方案中的 `TV` 可明确指向另一套已保存方案：按下后整套映射立即切换，并记住该目标作为下一次启动的当前配置。
...
向导分为“开始 / 权限与连接 / 控制偏好 / 按键配置 / 按键指南”五页：先选择“自动发送到 Codex”“启用遥控器按键控制”和可选的“登录时启动”，再处理 macOS、Whisper 与模型、米遥辅助功能、蓝牙、Codex 和安全启动组件的必要项；“按键配置”用于保存、选择和编辑自定义方案，“按键指南”内置默认映射图。只有“必须”和“当前功能必需”会阻止启动；关闭自动发送与按键控制后，辅助功能和 Codex 会变为可选。完整矩阵见 [权限与可选功能](docs/PERMISSIONS.md)。
