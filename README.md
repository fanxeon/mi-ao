...
| 交付形态 | **source-first beta · V2 / 0.2.0**；一条命令本地构建，安装后 App 内置日常运行组件，菜单栏 GUI 显示真实状态和安全操作 |
...
- **状态一直可见。** 菜单栏会按搜索、连接、录音、后台转写、发送结果和实体按键指令切换对应图标；指令执行时显示短暂蓝色圆角底色，成功为绿色、失败为红色。点击图标还可聚焦 Codex、打开记录、复查环境或安全退出。
...
核心真机语音链路已打通，当前是 **source-first beta · V2 / 0.2.0**：用户在自己的 Mac 上构建并使用 ad-hoc 签名 App。没有 Apple Developer ID 时，项目不会把未公证 DMG 包装成“一键安装”。
