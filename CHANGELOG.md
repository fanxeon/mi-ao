...
- 默认双控制模式：`TV` 在鼠标模式与方向键/Return/Escape 模式之间切换。
...
- 设备专属 `hidutil` 中性映射：只匹配小米遥控器 2 Pro，将方向键、确认、返回、HOME、TV、电源、语音和音量加减映射为 HID `No Event`；仅菜单保留原生行为。v1/v2/v3 旧映射可安全迁移和恢复。
