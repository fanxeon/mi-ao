...
双击会打开设置向导，而不是直接接管按键。向导复查六项环境后，“连接遥控器并开始”才调用真实启动门禁；成功后向导退出，米遥继续在菜单栏运行。当前没有开机自启。
...
双击 `~/Applications/米遥.app`，确认全部检查为绿色，点击“连接遥控器并开始”。命令行备用入口仍是 `./scripts/start.sh`。
...
- 已支持首次设置向导、菜单栏 GUI、安全后台启停和重复实例门禁；开机自启尚未实现；
