...
- `MenuBarPresentation.swift` / `MenuBarController.swift`：把基础运行态和 1.2 秒短暂指令态合成一个菜单栏呈现；动作图标、成功/失败色与圆角底色来自执行器的真实结果，录音、处理、断连和错误优先；popover 提供聚焦 Codex、打开记录、设置诊断和安全退出入口。
