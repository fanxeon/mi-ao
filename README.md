...
| 系统权限 | 蓝牙权限用于读取遥控器；辅助功能权限用于安全提交和执行按键动作 |
...
| 交付形态 | **source-first alpha**；本地构建、ad-hoc 签名，一条命令后台启动，菜单栏显示状态并安全退出 |
...
- **状态一直可见。** 菜单栏显示搜索、连接、录音、后台转写和发送结果，并可安全退出恢复遥控器。
...
该脚本在后台运行启动门禁，确认辅助功能权限和按键运行时可用；检查失败时不会修改系统。通过后，它从同一份内置硬件档案生成十二键 `No Event` 映射，阻止 macOS 前台 App 同时执行原生键盘行为；菜单继续作为系统鼠标右键。菜单栏中的“安全退出并恢复遥控器”会等待当前语音处理完成再退出并恢复原映射，也可运行 `./scripts/stop.sh`。
...
安装完成后的菜单栏状态、连续语音、仅转写模式、项目术语、更新和数据清理见 [完整使用说明](docs/USAGE.md)。双击 App 不会执行按键启动门禁，仍不作为推荐入口。
...
已经补齐菜单栏状态、安全后台启停、重复实例门禁、非阻塞转写队列和剪贴板并发保护。下一阶段聚焦：
