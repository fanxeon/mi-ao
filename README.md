...
> **状态边界：** 映射架构、双控制模式和执行器已经实现；语音链路、返回键以及 `TV` / 电源键的物理 Usage 已有真机证据，其中 `TV=0x07/0x35`、电源键 `0x07/0x66` 已按新格式人工确认。方向四键、确认、返回六项完整校准和动作执行验收仍未完成，因此实体按键动作目前是 implementation preview。
