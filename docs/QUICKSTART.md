...
设置向导会显示 macOS 版本、本地语音引擎、米遥辅助功能、蓝牙权限、Codex 输入区和启动组件。逐项点击需要处理的卡片，直到全部变为绿色。

辅助功能必须授权已安装的“米遥”，不要授权 `.build` 里的临时二进制。Codex 已关闭时，启动门禁会用自带的 Chromium 辅助功能参数打开它；Codex 正在工作且缺少参数时，向导只提示，不会自动重启。点击“准备 Codex”后还会再次要求你明确确认。参数只影响本次进程，不修改偏好设置、不开放调试端口。

## 4A. 在向导中启动已验证的小米 2 Pro

点击“连接遥控器并开始”。向导调用与 `./scripts/start.sh` 相同的真实启动门禁，成功后自动退出，米遥状态图标留在菜单栏。
