...
- 实体方向上执行时，菜单栏真实显示“移动鼠标 · 上”；指令使用蓝色圆角底色，成功为绿色、失败为红色，录音/转写/断连状态保持更高优先级。
- 设备选择与固定、确定性多设备仲裁、能力协商看门狗、可见分级重连、按键配置热更新/高亮/单次测试/安全导入导出均已进入真实运行合同。
...
| 交付形态 | **source-first beta · V2 / 0.2.1**；一条命令本地构建，安装后 App 内置日常运行组件，菜单栏 GUI 显示真实状态和安全操作 |
...
- **运行状态常驻。** 菜单栏会按搜索、连接、录音、后台转写、发送结果和实体按键指令切换对应图标；指令执行时显示短暂蓝色圆角底色，成功为绿色、失败为红色。点击图标还可聚焦 Codex、打开记录、复查环境或安全退出。刘海屏在右侧常驻项过多时可能遮挡状态项，这是可见区域限制，不代表米遥未运行；见 [故障排查](docs/TROUBLESHOOTING.md)。
...
如果遥控器已经连接但一次 ATVV 能力通知丢失，米遥会重试能力协商；连续 3 次无响应后会显示原因、主动断开并按分级退避重连。每次重连会先复用已保存设备和 macOS 仍保持连接的 ATVV 设备，再回退到广播扫描，不会无限停在“连接中”或漏掉停止广播的遥控器。
...
核心真机语音链路已打通，当前是 **source-first beta · V2 / 0.2.1**：用户在自己的 Mac 上构建并使用 ad-hoc 签名 App。没有 Apple Developer ID 时，项目不会把未公证 DMG 包装成“一键安装”。

V2 已完成 Preferences v2、功能分级权限、真实设备扫描/选择/持久化、多设备确定性仲裁、有反馈的断线重连、按键配置热更新/高亮/单次测试/安全导入导出，以及原子安装回滚和语音模型完整性校验。当前小米 2 Pro 固件 2671 的最终安装、全功能 BLE、实体方向/确认/TV/语音事件、菜单栏瞬时反馈和异常恢复已完成现场收口；逐项证据见 [V2 交付审计](docs/V2_COMPLETION_AUDIT.md) 与 [开发进度快照](docs/DEVELOPMENT_STATUS.md)。
