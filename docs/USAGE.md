...
完整两层映射示意图、默认按键表和安全边界见 [按键预设与默认指针模式](BUTTON_PRESETS.md)。六个必需按钮已完成真机校准，方向上已完成真实鼠标坐标验收；其余动作仍属于 implementation preview。
...
- 语音键已完成真机端到端验收；六个必需按钮已完成校准，方向上已完成鼠标动作闭环，其余实体动作仍在 [路线图](ROADMAP.md)；
