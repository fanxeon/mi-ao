...
- 实体按键模式会合成鼠标、方向键、Return 和 Escape。已确认的八个键会先通过设备专属 HID `No Event` 映射阻止原生键进入前台 App；事件关联过滤仍作为额外防线。点击、电源/Consumer Control、多显示器和同时键盘输入尚未完成全部真机验收。敏感编辑工作中请使用 `--no-buttons`。
