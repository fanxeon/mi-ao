...
校准只识别实体按钮，映射套装决定按钮用途。默认 `pointer` 套装同时覆盖指针控制和 Codex 会话导航；未来更换套装也无需重新校准硬件。
...
> **状态边界：** 映射架构、双控制模式和执行器已经实现；小米 2 Pro 固件 2671 的方向四键、确认、返回、HOME、TV、电源、语音和音量加减均已按新格式人工确认。米遥接管这十二键并拦截原生副作用，仅菜单保留 macOS 原生行为。音量加减切换 Codex 会话已完成双向真机验收；点击、模式切换和电源动作仍需逐项验收。
...
该脚本只为这款遥控器把方向键、确认、返回、HOME、TV、电源、语音和音量加减临时映射为 `No Event`，阻止 macOS 前台 App 同时执行原生键盘行为；仅菜单保持原生，米遥仍读取被接管键的原始 Usage。按 `Control + C` 停止并自动恢复原映射。
...
- 菜单栏状态与启停反馈；
- 设备选择、配置持久化与自动重连；
- 默认 pointer 套装点击、模式切换、电源动作和多显示器定位体验验收；
- Codex 会话导航等第二套映射；
- 更多真实遥控器的兼容矩阵；
- 可配置输出目标，但不弱化默认安全边界。
