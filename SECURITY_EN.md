...
- Physical-button mode takes over twelve keys: D-pad, Center, Back, HOME, TV, Power, Voice, and Volume Up/Down. Exact-device HID `No Event` prevents native side effects; only Menu remains native. MI-AO creates no global keyboard event tap and does not intercept the Mac's physical keyboard. Use `--no-buttons` during sensitive editing.
