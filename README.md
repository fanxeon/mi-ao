...
| 调试与安全 | 按设备精确校准 HID Usage；十二个接管键仅对目标遥控器中性化，退出时自动恢复；菜单键沿用 macOS 原生鼠标右键 |
| 交付形态 | **source-first alpha**；本地构建、ad-hoc 签名，当前通过终端脚本启动 |
...
该脚本只为这款遥控器把方向键、确认、返回、HOME、TV、电源、语音和音量加减共十二键临时映射为 `No Event`，阻止 macOS 前台 App 同时执行原生键盘行为；菜单不进入映射并继续作为系统鼠标右键，米遥仍读取被接管键的原始 Usage。按 `Control + C` 停止并自动恢复原映射。
