...
V2 会在安装更新前拒绝运行中 App，先校验分期 App 签名再原子替换，中途失败自动回滚。Whisper 模型下载到临时文件，只在固定 SHA-256 校验通过后替换。
...
回到向导的“权限与连接”，点击“扫描遥控器”查看真实 CoreBluetooth 结果。只有一支遥控器时可保持自动仲裁；多支或需固定时，在下拉框选择目标，UUID 会保存到私有偏好并优先重连。
...
点击“连接遥控器并开始”。向导调用 App 内置、与 `./scripts/start.sh` 同源的真实启动门禁；成功后会明确显示“米遥已在菜单栏运行”，点击“完成”后向导退出，米遥状态图标留在菜单栏。
