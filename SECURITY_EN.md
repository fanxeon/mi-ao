...
- Default pointer mode loads only user-confirmed profiles matching the remote Vendor/Product. Incomplete profiles, duplicate Usage assignments, or missing Accessibility disables physical-button actions while voice continues.
...
- Physical-button mode takes over ten keys: D-pad, Center, Back, HOME, TV, Power, and Voice. Exact-device HID `No Event` prevents native side effects; Menu and Volume Up/Down are never overridden. MI-AO creates no global keyboard event tap and does not intercept the Mac's physical keyboard. Use `--no-buttons` during sensitive editing.
